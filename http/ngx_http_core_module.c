    ...
    ngx_http_phase_handler_t *ph)
{
	...
	NGX_LOG(LOG_DEBUG, "no handler found");
    ngx_http_finalize_request(r, NGX_HTTP_NOT_FOUND);
    return NGX_OK;
}
...
ngx_http_output_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
	...
	NGX_LOG(LOG_DEBUG, "http output filter \"%s\"", r->uri.data);
    ...
    ngx_http_post_subrequest_t *ps, ngx_uint_t flags)
{
	...
	NGX_LOG(LOG_DEBUG, "http subrequest \"%s?\"", uri->data);
    ...
    ngx_str_t *uri, ngx_str_t *args)
{
	...
	NGX_LOG(LOG_DEBUG, "internal redirect: \"%s\"", uri->data);
...
ngx_http_core_directio(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    if (clcf->directio == (off_tt) NGX_ERROR) {
