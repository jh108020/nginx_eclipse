...
ngx_chain_t *
ngx_wsasend_chain(ngx_connection_t *c, ngx_chain_t *in, off_tt limit)
{
    int           rc;
    u_char       *prev;
    ...
    for ( ;; ) {
		...
		NGX_LOG(LOG_DEBUG, "WSASend: fd:%d, s:%ul", c->fd, sent);
...
ngx_chain_t *
ngx_overlapped_wsasend_chain(ngx_connection_t *c, ngx_chain_t *in, off_tt limit)
{
    int               rc;
    u_char           *prev;
	...
	NGX_LOG(LOG_DEBUG, "wev->complete: %d", wev->complete);
	...
	NGX_LOG(LOG_DEBUG, "WSASend ovlp: fd:%d, s:%ul", c->fd, sent);
