...
ngx_http_ssl_handshake(ngx_event_t *rev)
{
	...
	NGX_LOG(LOG_DEBUG, "http check ssl handshake");
...
ngx_http_ssl_servername(ngx_ssl_conn_t *ssl_conn, int *ad, void *arg)
{
	...
	NGX_LOG(LOG_DEBUG, "SSL server name: \"%s\"", servername);
...
ngx_http_process_request_line(ngx_event_t *rev)
{
	...
	NGX_LOG(LOG_DEBUG,  "http process request line");
    ...
    for ( ;; ) {
        ...
        if (rc == NGX_OK) {
            ...
            ngx_log_debug1(NGX_LOG_DEBUG_HTTP, c->log, 0,
                           "http uri: \"%V\"", &r->uri);
			NGX_LOG(LOG_DEBUG, "http uri: \"%s\"", r->uri.data);
    ...
    for ( ;; ) {
        ...
        if (rc == NGX_OK) {
            ...
            ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                           "http header: \"%V: %V\"",
                           &h->key, &h->value);
//			NGX_LOG(LOG_DEBUG, "http header: \"%s: %s\"",
//                           h->key.data, h->value.data);
        ...
        if (rc == NGX_HTTP_PARSE_HEADER_DONE) {
            ...
            ngx_log_debug0(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                           "http header done");
			NGX_LOG(LOG_DEBUG, "http header done");
    ...
    ngx_uint_t request_line)
{
	...
	NGX_LOG(LOG_DEBUG, "http alloc large header buffer");
...
ngx_http_request_handler(ngx_event_t *ev)
{
	...
	NGX_LOG(LOG_DEBUG, "http run request: \"%s\"", r->uri.data);
...
ngx_http_run_posted_requests(ngx_connection_t *c)
{
    ...
    for ( ;; ) {
		...
		NGX_LOG(LOG_DEBUG, "http posted request: \"%s?%s\"", r->uri.data, r->args.data);
...
ngx_http_finalize_request(ngx_http_request_t *r, ngx_int_t rc)
{
	...
	NGX_LOG(LOG_DEBUG, "http finalize request: %d, \"%s\" a:%d, c:%d",
                   rc, r->uri.data, r == c->data, r->main->count);
...
ngx_http_terminate_request(ngx_http_request_t *r, ngx_int_t rc)
{
	...
	NGX_LOG(LOG_DEBUG, "http terminate request count:%d", mr->count);
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, r->connection->log, 0,
                   ...
                   mr->count, mr->blocked);
	NGX_LOG(LOG_DEBUG,  "http terminate cleanup count:%d blk:%d",
                   mr->count, mr->blocked);
...
ngx_http_terminate_handler(ngx_http_request_t *r)
{
	...
	NGX_LOG(LOG_DEBUG, "http terminate handler count:%d", r->count);
...
ngx_http_writer(ngx_http_request_t *r)
{
	...
	NGX_LOG(LOG_DEBUG, "http writer handler: \"%s\"", r->uri.data);
    ...
    if (wev->timedout) {
        ...
        if (wev->delayed || r->aio) {
            ngx_log_debug0(NGX_LOG_DEBUG_HTTP, wev->log, 0,
                           "http writer delayed");
			NGX_LOG(LOG_DEBUG, "http writer delayed");
    ...
    ngx_log_debug3(NGX_LOG_DEBUG_HTTP, c->log, 0,
                   ...
                   rc, &r->uri, &r->args);
	NGX_LOG(LOG_DEBUG, "http writer output filter: %d, \"%s\"",
                   rc, r->uri.data);
	...
	NGX_LOG(LOG_DEBUG, "http writer done: \"%s\"", r->uri.data);
...
ngx_http_request_finalizer(ngx_http_request_t *r)
{
	...
	NGX_LOG(LOG_DEBUG, "http finalizer done: \"%s?%s\"", r->uri.data, r->args.data);
...
ngx_http_block_reading(ngx_http_request_t *r)
{
	...
	NGX_LOG(LOG_DEBUG, "http reading blocked");
...
ngx_http_test_reading(ngx_http_request_t *r)
{
	...
	NGX_LOG(LOG_DEBUG, "http test reading");
...
ngx_http_set_keepalive(ngx_http_request_t *r)
{
	...
	NGX_LOG(LOG_DEBUG, "set http keepalive handler");
    ...
    if (b->pos < b->last) {
		...
		NGX_LOG(LOG_DEBUG, "pipelined request");
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, c->log, 0, "hc free: %p %d",
                   hc->free, hc->nfree);
	NGX_LOG(LOG_DEBUG, "hc free: %p %d", hc->free, hc->nfree);
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, c->log, 0, "hc busy: %p %d",
                   hc->busy, hc->nbusy);
	NGX_LOG(LOG_DEBUG, "hc busy: %p %d", hc->busy, hc->nbusy);
...
ngx_http_keepalive_handler(ngx_event_t *rev)
{
	...
	NGX_LOG(LOG_DEBUG, "http keepalive handler");
...
ngx_http_lingering_close_handler(ngx_event_t *rev)
{
	...
	NGX_LOG(LOG_DEBUG, "http lingering close handler");
    ...
    do {
		...
		NGX_LOG(LOG_DEBUG, "lingering read: %d", n);
...
ngx_http_empty_handler(ngx_event_t *wev)
{
	...
	NGX_LOG(LOG_DEBUG, "http empty handler");
...
ngx_http_request_empty_handler(ngx_http_request_t *r)
{
	...
	NGX_LOG(LOG_DEBUG, "http request empty handler");
...
ngx_http_post_action(ngx_http_request_t *r)
{
	...
	NGX_LOG(LOG_DEBUG, "post action: \"%V\"", &clcf->post_action);
...
ngx_http_close_request(ngx_http_request_t *r, ngx_int_t rc)
{
	...
	NGX_LOG(LOG_DEBUG, "http request count:%d blk:%d", r->count, r->blocked);
...
ngx_http_free_request(ngx_http_request_t *r, ngx_int_t rc)
{
	...
	NGX_LOG(LOG_DEBUG, "http close request");
...
ngx_http_close_connection(ngx_connection_t *c)
{
	...
	NGX_LOG(LOG_DEBUG, "close http connection: %d", c->fd);
