...
ssize_t
ngx_read_file(ngx_file_t *file, u_char *buf, size_t size, off_tt offset)
{
    u_long      n;
    ngx_err_t   err;
...
ssize_t
ngx_write_file(ngx_file_t *file, u_char *buf, size_t size, off_tt offset)
{
    u_long      n;
    OVERLAPPED  ovlp, *povlp;
...
ssize_t
ngx_write_chain_to_file(ngx_file_t *file, ngx_chain_t *cl, off_tt offset,
    ngx_pool_t *pool)
{
    u_char   *buf, *prev;
    size_t    size;
...
ngx_create_file_mapping(ngx_file_mapping_t *fm)
{
    ...
    fm->handle = CreateFileMapping(fm->fd, NULL, PAGE_READWRITE,
                                   (u_long) ((off_tt) fm->size >> 32),
                                   (u_long) ((off_tt) fm->size & 0xffffffff),
                                   NULL);
    if (fm->handle == NULL) {
        ngx_log_error(NGX_LOG_CRIT, fm->log, ngx_errno,
