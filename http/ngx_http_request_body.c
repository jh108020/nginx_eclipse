    ...
    ngx_http_client_body_handler_pt post_handler)
{
    ...
    if (preread) {
		...
		NGX_LOG(LOG_DEBUG, "http client request body preread %uz", preread);
        ...
        if ((off_tt) preread >= r->headers_in.content_length_n) {
        ...
        if (rb->rest <= (off_tt) (b->end - b->last)) {
...
ngx_http_do_read_client_request_body(ngx_http_request_t *r)
{
	...
	NGX_LOG(LOG_DEBUG, "http read client request body");
    ...
    for ( ;; ) {
        for ( ;; ) {
			...
			NGX_LOG(LOG_DEBUG, "http client request body recv %z", n);
		...
		NGX_LOG(LOG_DEBUG, "http client request body rest %O", rb->rest);
...
ngx_http_discard_request_body(ngx_http_request_t *r)
{
	...
	NGX_LOG(LOG_DEBUG, "http set discard body");
...
ngx_http_read_discarded_request_body(ngx_http_request_t *r)
{
	...
	NGX_LOG(LOG_DEBUG, "http read discarded body");
