...
static ngx_int_t
ngx_output_chain_align_file_buf(ngx_output_chain_ctx_t *ctx, off_tt bsize)
{
    size_t      size;
    ngx_buf_t  *in;
...
static ngx_int_t
ngx_output_chain_get_buf(ngx_output_chain_ctx_t *ctx, off_tt bsize)
{
    size_t       size;
    ngx_buf_t   *b, *in;
    ngx_uint_t   recycled;
...
ngx_chain_writer(void *data, ngx_chain_t *in)
{
	...
	NGX_LOG(LOG_DEBUG, "chain writer in: %p", ctx->out);
	...
	NGX_LOG(LOG_DEBUG, "chain writer out: %p", ctx->out);
