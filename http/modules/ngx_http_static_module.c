...
ngx_http_static_handler(ngx_http_request_t *r)
{
	...
	NGX_LOG(LOG_DEBUG, "http filename: \"%s\"", path.data);
        ...
        != NGX_OK)
    {
        ...
        if (rc != NGX_HTTP_NOT_FOUND || clcf->log_not_found) {
			...
			NGX_LOG(LOG_ERROR, "%s \"%s\" failed", of.failed, path.data);
        }
	...
	NGX_LOG(LOG_DEBUG, "http static fd: %d", of.fd);

    if (of.is_dir) {

        ngx_log_debug0(NGX_LOG_DEBUG_HTTP, log, 0, "http dir");
		NGX_LOG(LOG_DEBUG, "http dir");
