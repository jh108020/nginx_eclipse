...
ngx_http_upstream_cache(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
	...
	NGX_LOG(LOG_DEBUG, "http upstream cache: %i", rc);
...
ngx_http_upstream_handler(ngx_event_t *ev)
{
	...
	NGX_LOG(LOG_DEBUG, "http upstream request: \"%s\"", &r->uri);
    ...
    ngx_event_t *ev)
{
    ...
    ngx_log_debug2(NGX_LOG_DEBUG_HTTP, ev->log, 0,
                   ...
                   ev->write, &r->uri);
	NGX_LOG(LOG_DEBUG, "http upstream check client, write event:%d, \"%s\"",
                   ev->write, r->uri.data);
	...
	NGX_LOG(LOG_DEBUG, "http upstream recv(): %d", n);
...
ngx_http_upstream_connect(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
	...
	NGX_LOG(LOG_DEBUG, "http upstream connect: %i", rc);
...
ngx_http_upstream_send_request(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
	...
	NGX_LOG(LOG_DEBUG, "http upstream send request");
    ...
    ngx_http_upstream_t *u)
{
	...
	NGX_LOG(LOG_DEBUG, "http upstream send request handler");
...
ngx_http_upstream_process_header(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
	...
	NGX_LOG(LOG_DEBUG, "http upstream process header");
    ...
    ngx_http_upstream_t *u)
{
	...
	NGX_LOG(LOG_DEBUG, "http upstream process body on memory");
...
ngx_http_upstream_send_response(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
	...
	NGX_LOG(LOG_DEBUG, "http cacheable: %d", u->cacheable);
...
ngx_http_upstream_process_non_buffered_downstream(ngx_http_request_t *r)
{
	...
	NGX_LOG(LOG_DEBUG, "http upstream process non buffered downstream");
    ...
    ngx_http_upstream_t *u)
{
	...
	NGX_LOG(LOG_DEBUG, "http upstream process non buffered upstream");
...
ngx_http_upstream_process_downstream(ngx_http_request_t *r)
{
	...
	NGX_LOG(LOG_DEBUG, "http upstream process downstream");
    ...
    ngx_http_upstream_t *u)
{
	...
	NGX_LOG(LOG_DEBUG, "http upstream process upstream");
...
ngx_http_upstream_process_request(ngx_http_request_t *r)
{
    ...
    if (u->peer.connection) {
        ...
        if (p->upstream_done || p->upstream_eof || p->upstream_error) {
			...
			NGX_LOG(LOG_DEBUG, "http upstream exit: %p", p->out);
#if 0
    ...
    if (p->downstream_error) {
		...
		NGX_LOG(LOG_ERROR, "http upstream downstream error");
...
ngx_http_upstream_store(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
	...
	NGX_LOG(LOG_DEBUG, "upstream stores \"%s\" to \"%s\"",
                   tf->file.name.data, path.data);
...
ngx_http_upstream_dummy_handler(ngx_http_request_t *r, ngx_http_upstream_t *u)
{
	...
	NGX_LOG(LOG_DEBUG, "http upstream dummy handler");
}
    ...
    ngx_uint_t ft_type)
{
	...
	NGX_LOG(LOG_DEBUG, "http next upstream, %xi", ft_type);
    ...
    if (u->peer.connection) {
		...
		NGX_LOG(LOG_DEBUG, "close http upstream connection: %d",
                       u->peer.connection->fd);
#if (NGX_HTTP_SSL)
...
ngx_http_upstream_cleanup(void *data)
{
	...
	NGX_LOG(LOG_DEBUG, "cleanup http upstream request: \"%s\"", r->uri.data);
    ...
    ngx_http_upstream_t *u, ngx_int_t rc)
{
	...
	NGX_LOG(LOG_DEBUG, "finalize http upstream request: %i", rc);
    ...
    if (u->pipe && u->pipe->temp_file) {
		...
		NGX_LOG(LOG_DEBUG, "http upstream temp fd: %d",
                       u->pipe->temp_file->file.fd);
    }
    ...
    *ho = *h;
    ...
    if (r->upstream->rewrite_redirect) {
        ...
        if (rc == NGX_OK) {
			...
			NGX_LOG(LOG_DEBUG, "rewritten location: \"%s\"", ho->value.data);
        }
    ...
    *ho = *h;
    ...
    if (r->upstream->rewrite_redirect) {
        ...
        if (rc == NGX_OK) {
			...
			NGX_LOG(LOG_DEBUG, "rewritten refresh: \"%s\"", ho->value.data);
        }
