...
ngx_http_script_start_args_code(ngx_http_script_engine_t *e)
{
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, e->request->connection->log, 0,
                   "http script args");
	NGX_LOG(LOG_DEBUG, "http script args");
...
ngx_http_script_regex_end_code(ngx_http_script_engine_t *e)
{
	...
	NGX_LOG(LOG_DEBUG, "http script regex end");
...
ngx_http_script_copy_capture_code(ngx_http_script_engine_t *e)
{
	...
	NGX_LOG(LOG_DEBUG, "http script capture: \"%*s\"", e->pos - pos, pos);
}
...
ngx_http_script_full_name_code(ngx_http_script_engine_t *e)
{
	...
	NGX_LOG(LOG_DEBUG, "http script fullname: \"%s\"", value.data);
...
ngx_http_script_if_code(ngx_http_script_engine_t *e)
{
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, e->request->connection->log, 0,
                   "http script if");
	NGX_LOG(LOG_DEBUG, "http script if");
	...
	NGX_LOG(LOG_DEBUG, "http script if: false");
...
ngx_http_script_equal_code(ngx_http_script_engine_t *e)
{
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, e->request->connection->log, 0,
                   "http script equal");
	NGX_LOG(LOG_DEBUG, "http script equal");
	...
	NGX_LOG(LOG_DEBUG, "http script equal: no");
...
ngx_http_script_not_equal_code(ngx_http_script_engine_t *e)
{
	...
	NGX_LOG(LOG_DEBUG, "http script not equal");
        ...
        && ngx_strncmp(val->data, res->data, res->len) == 0)
    {
		...
		NGX_LOG(LOG_DEBUG, "http script not equal: no");
...
ngx_http_script_file_code(ngx_http_script_engine_t *e)
{
	...
	NGX_LOG(LOG_DEBUG, "http script file op %p \"%s\"", code->op, path.data);
...
false_value:
	...
	NGX_LOG(LOG_DEBUG, "http script file op false");
...
ngx_http_script_complex_value_code(ngx_http_script_engine_t *e)
{
	...
	NGX_LOG(LOG_DEBUG, "http script complex value");
...
ngx_http_script_var_set_handler_code(ngx_http_script_engine_t *e)
{
	...
	NGX_LOG(LOG_DEBUG, "http script set var handler");
...
ngx_http_script_var_code(ngx_http_script_engine_t *e)
{
    ...
    ngx_log_debug0(NGX_LOG_DEBUG_HTTP, e->request->connection->log, 0,
                   "http script var");
	NGX_LOG(LOG_DEBUG, "http script var");
